...
**Start a fresh agent session per target.** Coding agents accumulate context across a session —
...
```bash
# Session 1: target A
bughunter autopilot targetA.com

# Session 2: target B (separate thread/process)
bughunter autopilot targetB.com
```
...
For long hunts, use the current agent's context-compaction feature periodically without
discarding persisted findings.
...
- Check the ledger (`~/.bughunter/memory/findings.jsonl`, with workspace-local `.bughunter/memory/` fallback) or run `bughunter memory-gc` for its size
