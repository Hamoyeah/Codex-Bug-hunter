...
Per-skill source and report-count provenance lives in `metadata/skill-provenance.json` so
Agent Skills frontmatter remains portable across Claude Code and Codex.
...
- **Bundle packaging** — Provider-specific installers for all 84 skills, Claude slash commands, the Codex `bughunter` router, and the hunt scaffold.
